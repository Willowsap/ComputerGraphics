...
public class Parser
{
	...
	private Object parseObject(Class<?> c, String text)
	{
		...
		else if (Tuple3.class.isAssignableFrom(c))
		{
			...
			try
			{
				result = (Tuple3) c.getDeclaredConstructor().newInstance();
			}
			catch (Exception e)
			{
		...
		else if (Color.class.isAssignableFrom(c))
		{
			...
			try
			{
				result = (Color) c.getDeclaredConstructor().newInstance();
			}
			catch (Exception e)
			{
				throw new Error("Error instantiating object of class: "
		...
		{
			ArrayList<Object> tempArray = parseArray(Integer.TYPE, text);
			if (tempArray.size() != 2)
			{
				throw new Error(
	...
	private Object parseObject(Class<?> c, Node n)
	{
		...
		// Otherwise the node represents a general object
			...
			// Create one!
			try {
				resultingObject = c.getDeclaredConstructor().newInstance();
			} catch (Exception e) {
				throw new Error("Error instantiating object of class: "
						+ c.getName());
			}
